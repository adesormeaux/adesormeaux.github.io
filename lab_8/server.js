...
app.get('/api', (req, res) => {
  //Change your server.js to request data from the Courses list at UMD.io
  const baseURL = 'https://api.umd.io/v0/courses/list';
  fetch(baseURL)
    .then((r) => r.json())
    .then((data) => {
      console.log(data);
      //Filter the returned courses data to only list courses with INST in the course_id. There are 68.
      const filtered = [];
      ...
      for(let i = 0; i < filtered.length; i++){
        fData.push([filtered[i].course_id,filtered[i].name].join(": "))
      }
